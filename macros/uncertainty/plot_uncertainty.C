...
void plot_uncertainty(){
  ...
  c->cd();
  ph_eta_up->GetXaxis()->SetTitle("p_T(#gamma)");
  ph_eta_up->GetYaxis()->SetTitle("Events");
  ph_eta_up->Draw("HIST E ");
  ph_eta_nominal->Draw("HIST SAME E ");
  ph_eta_down->Draw("HIST SAME E ");


  myText(  0.3,  0.85, 1, "#sqrt{s}= 13 TeV; W#gamma CR;");
